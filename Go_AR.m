...
% 2. For simplicity, rename the intial name suffix to 10.ibw

%% 1. Find good area (longest DB line > 55)
ARcmd(['DoScanFunc("DoScan_0")']);
pause(15);
file_order=Cin('BaseSuffix','GV');
...
while (isferro==0) % Repeat if the statement is true
    Scan_num = Scan_num + 1;
    
    if (Scan_num>=1) % if it is ESM
        run IsESM;
        return;
    end
...
%% 2. Zoom in and check this line (DB becomes longer)
